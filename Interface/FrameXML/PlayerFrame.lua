...
C_CVar:SetDefault("C_CVAR_PLAYER_FRAME_SHOW_DRAGON", true)
...
function PlayerFrame_UpdatePrimaryStat()
	...
	local spell, _, icon = C_PrimaryStat:GetPrimaryStatInfo(stat)
	local aura = C_PrimaryStat:GetPrimaryStatAura(stat)
	local tooltip
	if aura then
		tooltip = "spell:"..aura
	end
...
function PlayerFrame_ToVehicleArt(self, vehicleType)
	...
	if ( vehicleType == "Natural" ) then
		...
		PlayerFrameHealthBar:SetPoint("TOPLEFT",128,-41);
		PlayerFrameManaBar:SetWidth(103);
		PlayerFrameManaBar:SetPoint("TOPLEFT",128,-52);
	else
		PlayerFrameVehicleTexture:SetTexture("Interface\\Vehicles\\UI-Vehicle-Frame");
		PlayerFrameHealthBar:SetWidth(100);
		PlayerFrameHealthBar:SetPoint("TOPLEFT",131,-41);
		PlayerFrameManaBar:SetWidth(100);
		PlayerFrameManaBar:SetPoint("TOPLEFT",131,-52);
	end
...
function PlayerFrame_ToPlayerArt(self)
	...
	if classMask and bit.contains(bit.bor(Enum.ClassMask.HERO, Enum.ClassMask.COA), classMask) then
		local artwork = C_CVar:GetBool("C_CVAR_PLAYER_FRAME_SHOW_DRAGON") and "Interface\\TargetingFrame\\UI-HeroFrame-Dragon" or "Interface\\TargetingFrame\\UI-HeroFrame"
