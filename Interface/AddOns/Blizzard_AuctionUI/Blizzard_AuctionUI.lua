...
function BrowseButton_OnClick(button)
	...
	if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		DressUpItemLink(GetAuctionItemMysticEnchantLink("list", button:GetID() + FauxScrollFrame_GetOffset(BrowseScrollFrame)));
	end
...
function AuctionFrameBrowse_Update()
	...
	for i=1, NUM_BROWSE_TO_DISPLAY do
		...
		if ( index > (numBatchAuctions + (NUM_AUCTION_ITEMS_PER_PAGE * AuctionFrameBrowse.page)) ) then
			...
			local enchantID = GetAuctionItemMysticEnchant("list", offset + i)
			...
			if enchantID then
				button.Item.Enchant:Show()
				local RE = GetREData(enchantID)
				button.Item.Enchant.SpellID = RE.spellID
				button.Item.Enchant.Border:SetTexture(EnchantBorders[RE.quality])
				button.Item.Enchant.Highlight:SetTexture("Interface\\AddOns\\AwAddons\\Textures\\enchant\\EnchantBorder_highlight")
				local icon = select(3, GetSpellInfo(RE.spellID))
				button.Item.Enchant.Icon:SetPortraitTexture(icon or "Interface\\Icons\\inv_misc_questionmark")
			else
				button.Item.Enchant:Hide()
			end
...
function BidButton_OnClick(button)
	...
	if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		DressUpItemLink(GetAuctionItemMysticEnchantLink("bidder", button:GetID() + FauxScrollFrame_GetOffset(BidScrollFrame)));
	end
...
function AuctionsButton_OnClick(button)
	...
	if ( GetCVarBool("auctionDisplayOnCharacter") ) then
		DressUpItemLink(GetAuctionItemMysticEnchantLink("owner", button:GetID() + FauxScrollFrame_GetOffset(AuctionsScrollFrame)));
	end
